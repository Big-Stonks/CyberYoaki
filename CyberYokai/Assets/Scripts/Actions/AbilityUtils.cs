...
public class AbilityUtils : MonoBehaviour
{
	...
	public static class UserSelection
	{
		...
		/// <summary>
		/// Get a spot when user clicks on it.
		/// </summary>
		/// <param name="spot">Stores the spot to this variable</param>
		/// <param name="side">Which side is a valid side</param>
		/// <returns>Returns the clicked spot</returns>
		public static EntitySpot GetSpot(out EntitySpot spot, EntitySpot.SpotSide side)
		{
			EntitySpot s = null;
			Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
			if (Physics.Raycast(ray, out RaycastHit hit, 1000f, LayerMask.GetMask("Spot")))
			{
				if (hit.collider.GetComponent<EntitySpot>() != null)
				{
					EntitySpot checkingSpot = hit.collider.GetComponent<EntitySpot>();
					if (!checkingSpot.isOccupied)
					{
						if (Input.GetMouseButtonDown(0) && (checkingSpot.side == side || side == EntitySpot.SpotSide.Any))
						{
							s = checkingSpot;
						}
					}
